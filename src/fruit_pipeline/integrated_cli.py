...
INFERENCE_MODES = ("detector", "sam2_video")
...
def build_parser():
    ...
    parser.description = (
        ...
        "Two inference modes: 'detector' (default, see --inference-mode), the original detector + "
        "box-prompted-SAM2 pipeline; 'sam2_video' discovers automatically and propagates masks between "
        "refresh frames. The SAM2 variant is fixed by the container environment."
    )
    ...
    mode.add_argument(
        ...
        default="detector",
        help="'detector' (default): the original detector + box-prompted-SAM2 pipeline. "
        "'sam2_video': stateful SAM2 discovery and video propagation; configure "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.inference_mode == "detector":
